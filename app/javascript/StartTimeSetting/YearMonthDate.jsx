...
class YearMonthDate extends React.Component {
  render() {
    return (
      <span>
        <select id="schedule_start_time_1i" name="schedule[start_time(1i)]"
                className="browser-default inline-date" defaultValue={year}>
          <option value={year}>{year}</option>
          <option value={year + 1}>{year + 1}</option>
        </select>
        &ensp;
        <select id="schedule_start_time_2i" name="schedule[start_time(2i)]"
                className="browser-default inline-date" defaultValue={month}>
          {getOptions(1, 12, 'month')}
        </select>
        &ensp;
        <select id="schedule_start_time_3i" name="schedule[start_time(3i)]"
